class CartController < ApplicationController
  ...
  def add
    if current_user.nil?
      ...
      @electronic = Electronic.find_by(id: params[:id])
      quantity = params[:quantity].to_i
      current_orderable = @cart.orderables.find_by(electronic_id: @electronic.id)
      if current_orderable && quantity > 0
        current_orderable.update(quantity:)
      elsif quantity <= 0
        current_orderable.destroy
      else
        @cart.orderables.create(electronic: @electronic, quantity:)
      end
